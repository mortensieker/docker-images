...
set CONNECTION_URL=jdbc:mysql://$MYSQL_URI/datasource
...
#data-source add --name=mysqlDS --driver-name=mysql --jndi-name=java:jboss/datasources/MySQLDS --connection-url=jdbc:mysql://$MYSQL_HOST:$MYSQL_PORT/datasource --user-name=mysql --password=mysql --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
data-source add --name=mysqlDS --driver-name=mysql --jndi-name=java:jboss/datasources/MySQLDS --connection-url=jdbc:mysql://$MYSQL_URI/datasource --user-name=mysql --password=mysql --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
...
if [ "$ENABLE_ADMIN" = "on" ]; then
  echo "=> Adding admin user"
  /opt/jboss/wildfly/bin/add-user.sh --silent --user admin --password admin
  JBOSS_ADMIN="-bmanagement 0.0.0.0"
else
    JBOSS_ADMIN=
fi

echo "=> Restarting WildFly"
$JBOSS_HOME/bin/$JBOSS_MODE.sh -b 0.0.0.0 $JBOSS_ADMIN -c $JBOSS_CONFIG
